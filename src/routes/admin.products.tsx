...
const empty: Product = {
  ...
  price_kes: 0, compare_at_price_kes: null, stock: 0,
  category_id: null, image_url: "", featured: false,
};
...
function AdminProducts() {
  ...
  const uploadImage = async (file: File) => {
    ...
    try {
      ...
      const key = `${editing.slug || slugify(editing.name) || crypto.randomUUID()}-${Date.now()}.${ext}`;
      const { error } = await supabase.storage.from("product-images").upload(key, file, { upsert: true });
      if (error) throw error;
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        ...
        <Button variant="hero" onClick={() => setEditing({ ...empty })}><Plus className="h-4 w-4" /> New product</Button>
      </div>
              ...
              <tr key={p.id} className="border-t border-border">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    {p.image_url ? <img src={p.image_url} alt="" className="h-10 w-10 rounded object-cover" /> : <div className="h-10 w-10 rounded bg-secondary" />}
                    <div>
                ...
                <td className="px-4 py-3 text-right">
                  <button onClick={() => setEditing(p as Product)} className="mr-2 inline-flex h-8 w-8 items-center justify-center rounded hover:bg-secondary"><Pencil className="h-4 w-4" /></button>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/50 p-4" onClick={() => setEditing(null)}>
          <div className="my-8 w-full max-w-2xl rounded-2xl border border-border bg-card p-6" onClick={(e) => e.stopPropagation()}>
            <h2 className="font-display text-lg font-bold">{editing.id ? "Edit product" : "New product"}</h2>
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <Field label="Name" value={editing.name} onChange={(v) => setEditing({ ...editing, name: v, slug: editing.id ? editing.slug : slugify(v) })} />
              ...
              <Field label="Price (KES)" type="number" value={String(editing.price_kes)} onChange={(v) => setEditing({ ...editing, price_kes: Number(v) })} />
              <Field label="Compare-at price (KES)" type="number" value={String(editing.compare_at_price_kes ?? "")} onChange={(v) => setEditing({ ...editing, compare_at_price_kes: v ? Number(v) : null })} />
              ...
              <div className="md:col-span-2">
                ...
                <div className="mt-1 flex items-center gap-3">
                  {editing.image_url && <img src={editing.image_url} alt="" className="h-16 w-16 rounded object-cover" />}
                  <input
