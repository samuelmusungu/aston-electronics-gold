  ...
  head: () => ({
    meta: [
      { title: `Shop Electronics & Accessories - ${brand.name} Kenya` },
      {
        name: "description",
        content:
          "Browse phone cases, chargers, power banks, earphones and more. Buy online with M-Pesa.",
      },
    ],
  }),
  component: ShopPage,
  ...
  const { data: products = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from("products")
        .select(
          "id,name,slug,price_kes,compare_at_price_kes,image_url,brand,stock,categories!inner(slug)",
        );
